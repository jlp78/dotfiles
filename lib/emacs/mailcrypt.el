;; mailcrypt.el v1.6, mail encryption with RIPEM and PGP
...
;; 2-Jun-1994|1.6|~/interfaces/mailcrypt.el.Z|
...
;;   any fancy display stuff. Pick up show-temp-buffer.el if you liked the
...
(defun mc-deactivate-passwd ()
  ...
  (and (stringp mc-pgp-passwd) (fillarray mc-pgp-passwd 0))
  (and (stringp mc-ripem-passwd) (fillarray mc-pgp-passwd 0))
    ...
    (cond ((eq scheme 'pgp)
	   ...
	   (let ((process-environment process-environment))
	     ...
	     (setq retval (apply 'call-process-region
				 (nconc (list start (point-max) mc-pgp-path
					      t t nil)
					args)))
